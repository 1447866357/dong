...
class BallOnPlateEnv:
    def __init__(self):
        ...
        # gym placeholdert
        self.observation_space_shape = [12]
...
class BallOnPlate:
    ...
    def __init__(self, showGUI=False, randomInitial=False):
        ...
        p.setPhysicsEngineParameter(physicsClientId=self.physId)
        # p.setTimeStep(put_up. / 240)
        import time
        # time.sleep(10)
        ballRadius = 50 / 2 / 1000
        print('put_up', p.getPhysicsEngineParameters(physicsClientId=self.physId))
        ballMass = 0.200
    ...
    def state(self, ref_point):
        ...
        # print('put_up', ballvel)
        platepos, plateorn = p.getBasePositionAndOrientation(self.plateId, physicsClientId=self.physId)
        # print('2', platepos, plateorn)
        ls = p.getLinkState(bodyUniqueId=self.plateId,  linkIndex=1,computeLinkVelocity = 1,  physicsClientId=self.physId)
        ls1 = p.getLinkState(bodyUniqueId=self.plateId, linkIndex=0, computeLinkVelocity = 1, physicsClientId=self.physId)
        ls2 = p.getLinkState(bodyUniqueId=self.ballId,  linkIndex=0, computeLinkVelocity =1, physicsClientId=self.physId)
        # print('put_up', ls)
        # print('2', ls1)
        ## ball 不存在getLinkState
        # print('3', ls2)
        ballvel2 = p.getBaseVelocity(self.plateId, physicsClientId=self.physId)
        ...
        state = np.array(ballpos + ballvel[0][0:2] + platePos + plateOrn )
        # print('4', state)
        # print()
        return state
    ...
    def reward(self, state, done):
        ...
        if self.is_contacted() == 0:
            ...
            if done:
                ...
                r_step = 1
                if state[0] < 1e-4 and state[1] < 1e-4 and self.platehight - state[2] < 1e-1:
                    r_ball = 10
                    # print('aa')
                else:
                    r_ball = 1*math.exp(-(state[0] ** 2 + state[1] ** 2 ))
                r_plate = -10*(plateorn1[0] ** 2 + plateorn1[0] ** 2 + plateorn1[2] ** 2)

                reward = r_ball + r_plate + r_step
    ...
    def step(self, action, state):
        ...
        self.angleTargets = np.array(action)
        # print('put_up', action, self.angleTargets[0], self.angleTargets[put_up], )
        pos_prop = 0.01
        vel_prop = 1
        vel_max = 2
        force_max = 150
        # print('2', self.is_contacted())
        p.setJointMotorControl2(bodyUniqueId=self.plateId, jointIndex=0, controlMode=p.VELOCITY_CONTROL,

                                force = force_max, targetVelocity=self.angleTargets[0], physicsClientId=self.physId)

        p.setJointMotorControl2(bodyUniqueId=self.plateId, jointIndex=1, controlMode=p.VELOCITY_CONTROL,

                                force = force_max, targetVelocity = self.angleTargets[1], physicsClientId=self.physId)
    ...
    def _is_end(self, state):
        ...
        elif state[2] - 0.3 < 0 and self.is_contacted() == 0:
            done = True
        return done
