...
class BallOnPlate:
    ...
    def state(self, ref_point):
        ...
        state = np.array(ballpos + ballvel[0] + plateorn1)
        # print('2', state)
        return state
        ...
        if self.is_contacted() == 0:
            ...
            if done:
                ...
                if state[0] == 0 and state[1] == 0 and self.platehight - state[2] < 1e-2:
                    ...
                    r_ball = math.exp(-(state[0] ** 2 + state[1] ** 2))
                r_plate = -(state[6] ** 2 + state[7] ** 2 + state[8] ** 2)
    ...
    def _is_end(self, state):
        ...
        elif state[2] - 0.4 < 0 and self.is_contacted() == 0:
            done = True
        return done
